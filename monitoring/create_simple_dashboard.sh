...
# Create a simple dashboard with basic panels that we know will work
cat > simple_dashboard.json << 'EOF'
{
  "dashboard": {
    "id": null,
    "uid": null,
    "title": "Simple GPS Dashboard",
    "tags": ["gps"],
    "timezone": "browser",
    "schemaVersion": 38,
    "version": 0,
  ...
  "overwrite": true,
  "message": "Simple dashboard created via API"
}
EOF

# Import the dashboard via API
curl -X POST -H "Content-Type: application/json" -d @simple_dashboard.json http://admin:admin@localhost:3001/api/dashboards/db
