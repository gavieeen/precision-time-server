#!/bin/bash

# Create a targeted dashboard with specific queries that we know will work
cat > targeted_dashboard.json << 'EOF'
{
  "dashboard": {
    "id": null,
    "uid": null,
    "title": "GPS Data Dashboard",
    "tags": ["gps", "ptp"],
    "timezone": "browser",
    "schemaVersion": 38,
    "version": 0,
    "refresh": "5s",
    "time": {
      "from": "2025-05-09T00:00:00.000Z",
      "to": "2025-05-13T23:59:59.000Z"
    },
    "panels": [
      {
        "id": 1,
        "gridPos": {
          "h": 8,
          "w": 24,
          "x": 0,
          "y": 0
        },
        "type": "timeseries",
        "title": "PPS Clock Offset (ns)",
        "datasource": {
          "type": "influxdb",
          "uid": "P951FEA4DE68E13C9"
        },
        "fieldConfig": {
          "defaults": {
            "custom": {
              "drawStyle": "line",
              "lineInterpolation": "linear",
              "barAlignment": 0,
              "lineWidth": 1,
              "fillOpacity": 10,
              "gradientMode": "none",
              "spanNulls": false,
              "showPoints": "auto",
              "pointSize": 5,
              "stacking": {
                "mode": "none",
                "group": "A"
              },
              "axisLabel": "Offset (ns)",
              "scaleDistribution": {
                "type": "linear"
              }
            },
            "color": {
              "mode": "palette-classic"
            },
            "unit": "ns"
          }
        },
        "options": {
          "tooltip": {
            "mode": "single",
            "sort": "none"
          },
          "legend": {
            "displayMode": "table",
            "placement": "bottom",
            "calcs": ["mean", "max", "min"]
          }
        },
        "targets": [
          {
            "datasource": {
              "type": "influxdb",
              "uid": "P951FEA4DE68E13C9"
            },
            "query": "from(bucket: \"gps_data\")\n  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)\n  |> filter(fn: (r) => r._measurement == \"pps_data\")\n  |> filter(fn: (r) => r._field == \"offset_ns\")",
            "refId": "A"
          }
        ]
      },
      {
        "id": 2,
        "gridPos": {
          "h": 8,
          "w": 12,
          "x": 0,
          "y": 8
        },
        "type": "gauge",
        "title": "Satellites Visible",
        "datasource": {
          "type": "influxdb",
          "uid": "P951FEA4DE68E13C9"
        },
        "fieldConfig": {
          "defaults": {
            "min": 0,
            "max": 40,
            "thresholds": {
              "mode": "absolute",
              "steps": [
                {
                  "color": "red",
                  "value": null
                },
                {
                  "color": "yellow",
                  "value": 10
                },
                {
                  "color": "green",
                  "value": 20
                }
              ]
            }
          }
        },
        "options": {
          "orientation": "auto",
          "reduceOptions": {
            "values": false,
            "calcs": [
              "lastNotNull"
            ],
            "fields": ""
          },
          "showThresholdLabels": false,
          "showThresholdMarkers": true
        },
        "targets": [
          {
            "datasource": {
              "type": "influxdb",
              "uid": "P951FEA4DE68E13C9"
            },
            "query": "from(bucket: \"gps_data\")\n  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)\n  |> filter(fn: (r) => r._measurement == \"satellite_info\")\n  |> filter(fn: (r) => r._field == \"satellites_visible\")\n  |> last()",
            "refId": "A"
          }
        ]
      },
      {
        "id": 3,
        "gridPos": {
          "h": 8,
          "w": 12,
          "x": 12,
          "y": 8
        },
        "type": "gauge",
        "title": "Satellites Used",
        "datasource": {
          "type": "influxdb",
          "uid": "P951FEA4DE68E13C9"
        },
        "fieldConfig": {
          "defaults": {
            "min": 0,
            "max": 20,
            "thresholds": {
              "mode": "absolute",
              "steps": [
                {
                  "color": "red",
                  "value": null
                },
                {
                  "color": "yellow",
                  "value": 4
                },
                {
                  "color": "green",
                  "value": 8
                }
              ]
            }
          }
        },
        "options": {
          "orientation": "auto",
          "reduceOptions": {
            "values": false,
            "calcs": [
              "lastNotNull"
            ],
            "fields": ""
          },
          "showThresholdLabels": false,
          "showThresholdMarkers": true
        },
        "targets": [
          {
            "datasource": {
              "type": "influxdb",
              "uid": "P951FEA4DE68E13C9"
            },
            "query": "from(bucket: \"gps_data\")\n  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)\n  |> filter(fn: (r) => r._measurement == \"satellite_info\")\n  |> filter(fn: (r) => r._field == \"satellites_used\")\n  |> last()",
            "refId": "A"
          }
        ]
      },
      {
        "id": 4,
        "gridPos": {
          "h": 9,
          "w": 24,
          "x": 0,
          "y": 16
        },
        "type": "timeseries",
        "title": "Dilution of Precision (DOP)",
        "datasource": {
          "type": "influxdb",
          "uid": "P951FEA4DE68E13C9"
        },
        "fieldConfig": {
          "defaults": {
            "custom": {
              "drawStyle": "line",
              "lineInterpolation": "linear",
              "barAlignment": 0,
              "lineWidth": 1,
              "fillOpacity": 10,
              "gradientMode": "none",
              "spanNulls": false,
              "showPoints": "auto",
              "pointSize": 5,
              "stacking": {
                "mode": "none",
                "group": "A"
              },
              "axisLabel": "",
              "scaleDistribution": {
                "type": "linear"
              }
            },
            "color": {
              "mode": "palette-classic"
            }
          }
        },
        "options": {
          "tooltip": {
            "mode": "single",
            "sort": "none"
          },
          "legend": {
            "displayMode": "list",
            "placement": "bottom",
            "calcs": []
          }
        },
        "targets": [
          {
            "datasource": {
              "type": "influxdb",
              "uid": "P951FEA4DE68E13C9"
            },
            "query": "from(bucket: \"gps_data\")\n  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)\n  |> filter(fn: (r) => r._measurement == \"satellite_info\")\n  |> filter(fn: (r) => r._field == \"hdop\" or r._field == \"vdop\" or r._field == \"pdop\" or r._field == \"gdop\")",
            "refId": "A"
          }
        ]
      },
      {
        "id": 5,
        "gridPos": {
          "h": 8,
          "w": 24,
          "x": 0,
          "y": 25
        },
        "type": "timeseries",
        "title": "GPS Altitude",
        "datasource": {
          "type": "influxdb",
          "uid": "P951FEA4DE68E13C9"
        },
        "fieldConfig": {
          "defaults": {
            "custom": {
              "drawStyle": "line",
              "lineInterpolation": "linear",
              "barAlignment": 0,
              "lineWidth": 1,
              "fillOpacity": 10,
              "gradientMode": "none",
              "spanNulls": false,
              "showPoints": "auto",
              "pointSize": 5,
              "stacking": {
                "mode": "none",
                "group": "A"
              },
              "axisLabel": "Altitude (m)",
              "scaleDistribution": {
                "type": "linear"
              }
            },
            "color": {
              "mode": "palette-classic"
            },
            "unit": "m"
          }
        },
        "options": {
          "tooltip": {
            "mode": "single",
            "sort": "none"
          },
          "legend": {
            "displayMode": "list",
            "placement": "bottom",
            "calcs": []
          }
        },
        "targets": [
          {
            "datasource": {
              "type": "influxdb",
              "uid": "P951FEA4DE68E13C9"
            },
            "query": "from(bucket: \"gps_data\")\n  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)\n  |> filter(fn: (r) => r._measurement == \"gps_location\")\n  |> filter(fn: (r) => r._field == \"altitude\")",
            "refId": "A"
          }
        ]
      }
    ]
  },
  "overwrite": true,
  "message": "Targeted dashboard created via API"
}
EOF

# Import the dashboard via API
curl -X POST -H "Content-Type: application/json" -d @targeted_dashboard.json http://admin:admin@localhost:3001/api/dashboards/db

echo -e "\n\nDashboard created! Access it at: http://localhost:3001/dashboards"
